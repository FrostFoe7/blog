...
>) {
  const textColor = calloutConfig[variant].textColor
    ...
    >
      <summary className="flex cursor-pointer items-center font-medium [&::-webkit-details-marker]:hidden">
        <MdxIcon
          ...
          className={cn('mr-2 size-4 shrink-0', textColor)}
        />
        <span className={cn('font-medium mr-2', textColor)}>
          {variant.charAt(0).toUpperCase() + variant.slice(1)}
          {title ? <span className="font-normal opacity-70"> ({title})</span> : null}
        </span>
        <MdxIcon
          name="lucide:chevron-down"
          className={cn('ml-auto h-4 w-4 shrink-0 transition-transform duration-200', textColor)}
        />
      </summary>
      <div>{children}</div>
    </details>
